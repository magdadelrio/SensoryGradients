...
% define pathways
gradientdatadir = 'C:\Users\Magda\Documents\Matlab\rgs\gradients\'; % where to save FC matrix
rawdatadir = '\research\cisc2\projects\ward_restinggsq\dicom\'; % where is raw data saved
fmridir = '\research\cisc2\projects\ward_restinggsq\fmri\'; % where is the timeseries data saved
datafilesdir = 'C:\Users\Magda\Documents\Matlab\rgs\datafiles'; %intermediate data files saved here, to bypass custom-made functions
...
for pp=1:length(subjectid_batch)
    ...
    cd('\research\cisc2\projects\ward_restinggsq\scripts_batches\')
...
%         [roimask,roinames]=cvnroimask('fsaverage',hemis,atlasstr,[],surfsuffix,'collapsevals'); %extract atlas indices and names for a particular atlas and particular subject (in this case fsaverage)
    ...
    %     %% Examine mean timeseries in a few rois (not needed, just for sanity check)
%         close all
%         figure
%         for p=2:10%loop over the first 9 rois
%             subplot(3,3,p-1)
%             roimean=mean(lhsubsample(:,r),2);%index the data using the atlas mask and take the average of all vertices in the region
%             plot(roimean)%plot mean timeseries responses for each ROI
%             title(roinames_lh{p}(14:end))%title the plot by roi name
%         end
